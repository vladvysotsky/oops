...
выглядит настоящим, остаётся нетронутым: `appconfig` внутри русской фразы так и
останется `appconfig`, а не превратится в `фззсщташп`. Словаря при этом нет —
проверка идёт по статистике буквосочетаний, меньше двух килобайт, поэтому
`nginx` и `useState` тоже проходят.
