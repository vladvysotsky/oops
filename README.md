...
- **"Автоматически исправлять раскладку (бета)"** — на разделителе
  (пробел/знак препинания) анализируем последнее слово через словарь:
  если слово знакомо в исходном языке — оставляем; если после
  layout-конвертации становится знакомым в другом языке, а в исходном
  не знакомо — конвертируем. Для неизвестных слов — fallback на эвристику
  по гласным. Базовые словари (~500 EN + ~500 RU частотных слов) встроены
  в сборку. Их можно расширить:
  - `%AppData%\KeyLangSwitcher\words_en.user.txt`
  - `%AppData%\KeyLangSwitcher\words_ru.user.txt`
  Формат: одно слово в строке, в нижнем регистре, строки с `#` —
  комментарии.
