...
По умолчанию приложение запускается **без UAC** (asInvoker) — иначе
автозапуск через `HKCU\...\Run` молча игнорируется Windows на старте
системы. Во всех обычных приложениях (браузер, IDE, мессенджеры, Word,
терминал и т.п.) работает корректно.

Для конвертации в окнах, запущенных **от имени администратора** (RegEdit,
диспетчер задач, любые приложения с UAC-щитом), запускайте KeyLangSwitcher
вручную через "Run as administrator". Автозапуск в этом случае настроить
напрямую нельзя — Windows запретит. Альтернатива: создать Scheduled Task с
"Run with highest privileges" вручную через Task Scheduler.
