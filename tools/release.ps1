...
try {
    ...
    # --prune-tags нужен отдельно: одного --prune --tags не хватило, локальный
    #   тег пережил удаление серверного и остановил следующий запуск.
    ...
    #    номер совпадёт.
    #
    #    Локальный и серверный разделены НАМЕРЕННО. Тег на origin — это сгоревший
    #    номер, и делать с ним нечего. Тег только локально — это мусор от
    #    прошлой попытки, он ничего не защищает, и человеку нужна не констатация,
    #    а команда: без неё этот отказ стоил целого круга.
    if (git ls-remote --tags origin $tag) { throw "Тег $tag уже есть на origin — номер сгорел, выпускайте следующий" }
    if (git tag --list $tag) {
        throw "Тег $tag остался локально от прошлой попытки (на origin его нет). Удалите его и повторите: git tag -d $tag"
    }
