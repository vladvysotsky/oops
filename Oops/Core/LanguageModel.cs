...
/// буквы недостаточно — «xtuj» и «password» оба полностью латинские, и
/// отличить мусор от настоящего слова можно только зная язык. Раньше такой
/// текст не чинился ни одним нажатием: «password» превращался в «зфыыцщкв».
///
...
/// 784 байта на язык. «зфыыцщкв» отбраковывается не потому, что его нет в
/// списке, а потому, что сочетания «зз», «сщ», «шп» в русском почти не
/// встречаются. Незнакомое слово или имя — «Anderson», «Helsinki» — модель принимает
