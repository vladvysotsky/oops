...
- **Карточке (`Card`) ширина задаётся сразу при создании, до раскладки.** До
  первой раскладки у неё ширина по умолчанию (200 px), докнутые внутрь ряды
  переносят абзацы по двадцать раз, и карточка «вырастает» в несколько экранов.
  Высота вкладок меряется именно в этот момент (`TallestPageHeight`), окно
  фиксировалось по выдуманной высоте: содержимое потом сжималось до
  нормального, а под ним оставалась пустота во весь экран. Проявилось, когда на
  вкладку «Поведение» добавились карточки с длинными абзацами.
