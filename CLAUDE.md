...
- **Ряд кнопок — только через `ButtonBar` и только `Dock = Top`.** `Anchor` и
  `AutoSize` спорят за ширину: WinForms сжимает контейнер до предпочтительного
  размера, и правый край кнопок оказывается то за границей окна, то вплотную к
  ней. `Dock = Top` берёт ширину рабочей области родителя (она уже учитывает
  его `Padding`), а `AutoSize` остаётся только на высоту. На этом сожжено пять
  заходов.
