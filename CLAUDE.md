...
5. тег **только на `origin/main`** и только после этого:
   `git fetch origin && git tag -a vX.Y.Z origin/main -m "…" && git push origin vX.Y.Z`.
...
- `UI/WhatsNewForm.cs` + `Core/Changelog.cs` — «Что нового»: показывается один
