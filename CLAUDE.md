...
- `UI/SettingsForm.cs` — окно настроек. **Вкладки** (общие / хоткеи / поведение /
  проверка) на том же `SegmentedControl`, что и переключатель языка. Размер окна
  задан явно, `AutoSize` у формы НЕТ: пока высота была суммой всех секций, окно
  вырастало выше экрана, а подвал уезжал за край — вся раскладка висела на одной
  цепочке AutoSize. Корень — `TableLayoutPanel` с четырьмя строками, НЕ
  Dock.Fill+Dock.Bottom: порядок докинга в WinForms зависит от z-order.
